...
namespace X {
	...
	Texture::Ptr GLRenderer::LoadTexture(const String & source)
	{
		auto i = mTextureMap.find(source);
		if (i != mTextureMap.end())
			return i->second;

		GLTexture * tex = new GLTexture(source, source);
		tex->_load();
		mTextureMap.insert(std::pair<String, Texture *>(source, tex));

		return tex;
	}

	Texture::Ptr GLRenderer::CreateTexture(const String & name, int format, int w, int h, int mipmaps)
	{
		d_assert(mTextureMap.find(name) == mTextureMap.end());

		GLTexture * tex = new GLTexture(name, "");
		tex->mFormat = format;
		tex->mWidth = w;
		tex->mHeight = h;
		tex->mMipmaps = mipmaps;
		mTextureMap.insert(std::pair<String, Texture *>(name, tex));
